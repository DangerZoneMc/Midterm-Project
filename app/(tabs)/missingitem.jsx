...
export default function MissingItemScreen() {
  ...
  const validateForm = () => {
    ...
    if (!location.trim()) {
      Alert.alert('Error', 'Please enter the last known location');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await StorageService.saveMissingItem({
        itemName,
        selectedCategory,
        description,
        location,
      ...
      Alert.alert(
        'Success',
        'Your missing item has been reported successfully',
        [
          {
            text: 'View Items',
      ...
      console.error('Error submitting missing item:', error);
      Alert.alert('Error', 'Failed to submit missing item. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.formSection}>
            ...
            <TextInput
              ...
              placeholder="Enter where you last saw the item"
              placeholderTextColor="#666"
            />
